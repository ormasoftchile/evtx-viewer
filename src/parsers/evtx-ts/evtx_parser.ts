...
export class EvtxParser {
    ...
    constructor(data: Buffer, header: EvtxFileHeader, config: ParserSettings, calculated_chunk_count: number) {
        this.data = data;
        this.header = header;
        this.config = config;
    ...
    public static from_path(path: string): EvtxResult<EvtxParser> {
        try {
            ...
            if (chunk_data_size < 0) {
                return err(new Error(`Could not calculate valid chunk count because stream size is less than evtx header block size. (stream_size: ${stream_size}, header_block_size: ${evtx_header.header_block_size})`));
            }

            const chunk_count = Math.floor(chunk_data_size / EVTX_CHUNK_SIZE);

            return ok(new EvtxParser(
                file_data,
                evtx_header,
            ...
            return err(new Error(`Failed to open evtx file: ${e instanceof Error ? e.message : String(e)}`));
        }
    }

    // 1:1 translation of with_configuration method
    public with_configuration(configuration: ParserSettings): EvtxParser {
        return new EvtxParser(this.data, this.header, configuration.clone(), this.calculated_chunk_count);
    }
    ...
    public static from_buffer(buffer: Buffer): EvtxResult<EvtxParser> {
        try {
            ...
            return err(new Error(`Failed to parse evtx buffer: ${e instanceof Error ? e.message : String(e)}`));
        }
    }
    ...
    public* records(): Generator<EvtxResult<SerializedEvtxRecord<string>>, void, unknown> {
        ...
        for (let chunk_number = 0; chunk_number < this.calculated_chunk_count; chunk_number++) {
            const chunk_result = this.allocate_chunk(chunk_number, this.config.should_validate_checksums());
    ...
    public* records_json(): Generator<EvtxResult<SerializedEvtxRecord<string>>, void, unknown> {
        yield* this.serialized_records((record) => {
            ...
            return ok(new SerializedEvtxRecord(
            ...
            try {
                ...
                for (const record of records) {
                    try {
                        ...
                        yield ok(new SerializedEvtxRecord(
    ...
    private allocate_chunk(chunk_number: number, validate_checksum: boolean): EvtxResult<EvtxChunkData | null> {
        try {
            ...
            return err(new Error(`Failed to allocate chunk ${chunk_number}: ${e instanceof Error ? e.message : String(e)}`));
        }
    }
}
