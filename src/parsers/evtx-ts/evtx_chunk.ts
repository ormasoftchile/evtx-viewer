    ...
    public static from_reader(data: Buffer): Result<EvtxChunkHeader, Error> {
        try {
            ...
            if (!magic.equals(expected_magic)) {
                return err(new Error(`Invalid EVTX chunk magic: expected ${expected_magic.toString('hex')}, got ${magic.toString('hex')}`));
            }

            let offset = 8;

            // Direct translation: read chunk header fields in exact same order as Rust
            const first_event_record_number = data.readBigUInt64LE(offset); offset += 8;
            const last_event_record_number = data.readBigUInt64LE(offset); offset += 8;
            const first_event_record_id = data.readBigUInt64LE(offset); offset += 8;
            const last_event_record_id = data.readBigUInt64LE(offset); offset += 8;

            const header_size = data.readUInt32LE(offset); offset += 4;
            const last_event_record_data_offset = data.readUInt32LE(offset); offset += 4;
            const free_space_offset = data.readUInt32LE(offset); offset += 4;
            const events_checksum = data.readUInt32LE(offset); offset += 4;

            // Skip reserved 64 bytes
            offset += 64;

            const raw_flags = data.readUInt32LE(offset); offset += 4;
            const flags = ChunkFlags.from_bits_truncate(raw_flags);
            const header_chunk_checksum = data.readUInt32LE(offset); offset += 4;
            ...
            return err(new Error(`Failed to parse EVTX chunk header: ${e instanceof Error ? e.message : String(e)}`));
        }
    }
}
...
// console.log("Validating data checksum");
        ...
        const computed_checksum = checksum_disabled ? 0 : this.simple_checksum(
            ...
            try {
                ...
                if (binxml_data_size > 0 && binxml_data_offset + binxml_data_size <= this.data.length) {
                    ...
                    if (deserializer_result.kind === 'ok') {
                        ...
                        console.warn(`Failed to parse binary XML for record ${record_header.event_record_id}: ${deserializer_result.error.message}`);
                    }
                }
