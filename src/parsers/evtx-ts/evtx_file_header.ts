    ...
    public static from_stream(data: Buffer): Result<EvtxFileHeader, Error> {
        try {
            ...
            if (!magic.equals(expected_magic)) {
                return err(new Error(`Invalid EVTX file header magic: expected ${expected_magic.toString('hex')}, got ${magic.toString('hex')}`));
            }

            let offset = 8;

            // Direct translation: read header fields in exact same order as Rust
            const oldest_chunk = data.readBigUInt64LE(offset); offset += 8;
            const current_chunk_num = data.readBigUInt64LE(offset); offset += 8;
            const next_record_num = data.readBigUInt64LE(offset); offset += 8;
            const header_size = data.readUInt32LE(offset); offset += 4;
            const minor_version = data.readUInt16LE(offset); offset += 2;
            const major_version = data.readUInt16LE(offset); offset += 2;
            const header_block_size = data.readUInt16LE(offset); offset += 2;
            const chunk_count = data.readUInt16LE(offset); offset += 2;
            ...
            return err(new Error(`Failed to parse EVTX file header: ${e instanceof Error ? e.message : String(e)}`));
        }
    }
