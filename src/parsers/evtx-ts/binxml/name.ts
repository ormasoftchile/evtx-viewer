...
export class BinXmlName {
  ...
  static from_stream(cursor: Buffer, offset: number): Result<BinXmlName> {
    try {
      ...
      return err(
        new Error(`Failed to read BinXmlName: ${e instanceof Error ? e.message : String(e)}`)
      );
    }
  }
...
export class BinXmlNameRef {
  ...
  static from_stream(cursor: Buffer, position: number): ResultWithBytes<BinXmlNameRef> {
    try {
      ...
      return {
        result: err(
          new Error(`Failed to read BinXmlNameRef: ${e instanceof Error ? e.message : String(e)}`)
        ),
        bytes_consumed: 0,
      };
    }
...
export class BinXmlNameLink {
  ...
  static from_stream(cursor: Buffer, offset: number): Result<BinXmlNameLink> {
    try {
      ...
      return ok(new BinXmlNameLink(next_string > 0 ? next_string : null, name_hash));
    } catch (e) {
      return err(
        new Error(`Failed to read BinXmlNameLink: ${e instanceof Error ? e.message : String(e)}`)
      );
    }
  }
