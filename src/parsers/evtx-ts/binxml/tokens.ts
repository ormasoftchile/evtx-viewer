...
// console.log('\t Skipping `NullType` descriptor');
    ...
    return okWithBytes(new BinXmlTemplateRef(template_definition_data_offset, substitution_array), bytes_consumed);
}
...
export function read_template_definition_header(
    ...
    const bytes_consumed = pos - position;
    return okWithBytes(new BinXmlTemplateDefinitionHeader(next_template_offset, template_guid, data_size), bytes_consumed);
}
...
export function read_template_definition(
    ...
    if (fragment_result.kind === 'err') {
        return err(new Error(`Failed to deserialize template ${header.guid}: ${fragment_result.error.message}`));
    }
...
export function read_entity_ref(cursor: Buffer, position: number): ResultWithBytes<BinXmlEntityReference> {
...
export function read_fragment_header(cursor: Buffer, position: number): Result<BinXMLFragmentHeader> {
