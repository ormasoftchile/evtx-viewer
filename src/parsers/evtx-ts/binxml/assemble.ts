...
// console.log("DEBUG CloseStartElement: Pushing OpenElement to model");
            ...
            case 'PIData':
                ...
                const piData = token.kind === 'owned' ? owned(tokenValue.value) : borrowed(tokenValue.value);
...
function expand_string_ref(
    string_ref: BinXmlNameRef,
    chunk: any
): Result<Cow<BinXmlName>> {
...
function expand_token_substitution(
    ...
    if (value !== undefined) {
        ...
        template.substitution_array[substitution_descriptor.substitution_index] = 
            { kind: 'Value', value: { kind: 'NullType' } };
...
export function expand_templates(
    ...
    for (let i = 0; i < token_tree.length; i++) {
        ...
        const expandResult = _expand_templates(owned(token_tree[i]! as BinXMLDeserializedTokens), chunk, stack);
        if (expandResult.kind === 'err') {
            return expandResult;
        }
    }
