        ...
        const result = (() => {
            switch (raw_token.kind) {
                ...
                case 'OpenStartElement': {
                    ...
                    return ok({ kind: 'OpenStartElement', value: element_result.result.value } as BinXMLDeserializedTokens);
                }
                ...
                case 'Value': {
                    ...
                    return ok({ kind: 'Value', value: value_result.result.value } as BinXMLDeserializedTokens);
                }
                ...
                case 'Attribute': {
                    ...
                    return ok({ kind: 'Attribute', value: attr_result.result.value } as BinXMLDeserializedTokens);
                }

                case 'CDataSection':
                    return err(new Error(`Unimplemented token: CDataSection at offset ${this.cursor_position}`));

                case 'CharReference':
                    return err(new Error(`Unimplemented token: CharReference at offset ${this.cursor_position}`));
                ...
                case 'EntityReference': {
                    ...
                    return ok({ kind: 'EntityRef', value: entity_result.result.value } as BinXMLDeserializedTokens);
                }

                case 'ProcessingInstructionTarget': {
                    const pi_target_result = read_processing_instruction_target(this.cursor, this.cursor_position);
                    if (pi_target_result.result.kind === 'err') {
                        return pi_target_result.result;
                    }
                    this.cursor_position += pi_target_result.bytes_consumed;
                    return ok({ kind: 'PITarget', value: pi_target_result.result.value } as BinXMLDeserializedTokens);
                }

                case 'ProcessingInstructionData': {
                    const pi_data_result = read_processing_instruction_data(this.cursor, this.cursor_position);
                ...
                case 'TemplateInstance': {
                    ...
                    return ok({ kind: 'TemplateInstance', value: template_result.result.value } as BinXMLDeserializedTokens);
                }

                case 'NormalSubstitution': {
                    const subst_result = read_substitution_descriptor(this.cursor, this.cursor_position, false);
                    if (subst_result.kind === 'err') {
                        return subst_result;
                    }
                    this.cursor_position += 3; // Approximate advancement
                    return ok({ kind: 'Substitution', value: subst_result.value } as BinXMLDeserializedTokens);
                }

                case 'ConditionalSubstitution': {
                    const subst_result = read_substitution_descriptor(this.cursor, this.cursor_position, true);
                    if (subst_result.kind === 'err') {
                        return subst_result;
                    }
                    this.cursor_position += 3; // Approximate advancement
                ...
                case 'StartOfStream': {
                    ...
                    return ok({ kind: 'FragmentHeader', value: fragment_result.value } as BinXMLDeserializedTokens);
                }
...
export class BinXmlDeserializer {
    ...
    public static read_binxml_fragment(
        ...
        try {
            ...
            const de = BinXmlDeserializer.init(
                cursor,
                offset,
                chunk,
                is_inside_substitution,
            ...
            return err(new Error(`Failed to read binxml fragment: ${e instanceof Error ? e.message : String(e)}`));
        }
    }
    ...
    public iter_tokens(data_size: number | null): Result<IterTokens> {
        try {
            ...
            return err(new Error(`Failed to create token iterator: ${e instanceof Error ? e.message : String(e)}`));
        }
    }
}