...
export function binxml_value_from_stream(
    ...
    try {
        ...
        if (value_type === null) {
            ...
            return err(new Error(`Invalid value variant: ${value_type_token} at offset ${cursor_position}`));
        }
        ...
        return err(new Error(`Failed to read value from stream: ${e instanceof Error ? e.message : String(e)}`));
    }
}
...
export function deserialize_value_type(
    ...
    try {
        ...
        return err(new Error(`Failed to deserialize value type: ${e instanceof Error ? e.message : String(e)}`));
    }
}
