        ...
        for (const item of model) {
            switch (item.kind) {
                case 'OpenElement':
                    ...
                    const element_name_obj = element.name.kind === 'owned' ? element.name.value : element.name.value;
                    ...
                    if (element.attributes && element.attributes.length > 0) {
                        ...
                        for (const attr of element.attributes) {
                            ...
                            const attr_value_obj = attr.value.kind === 'owned' ? attr.value.value : attr.value.value;
                            ...
                            } else if (attr_value_obj.kind === 'SysTimeType' || attr_value_obj.kind === 'FileTimeType') {
                            ...
                            } else if (attr_value_obj.kind === 'UInt32Type' || attr_value_obj.kind === 'Int32Type' ||
                                       attr_value_obj.kind === 'UInt16Type' || attr_value_obj.kind === 'Int16Type' ||
                                       attr_value_obj.kind === 'UInt8Type' || attr_value_obj.kind === 'Int8Type') {
                                // Keep as number for 32-bit and smaller numeric types
                                attr_value = attr_value_obj.value;
                            } else if (attr_value_obj.kind === 'UInt64Type' || attr_value_obj.kind === 'Int64Type') {
                ...
                case 'CloseElement':
                    if (stack.length > 0) {
                        ...
                        else if (closing_element_name === 'Data' && current['#attributes'] && current['#attributes']['Name']) {
                ...
                case 'Value':
                    ...
                    } else if (value_obj.kind === 'UInt32Type' || value_obj.kind === 'Int32Type' ||
                               value_obj.kind === 'UInt16Type' || value_obj.kind === 'Int16Type' ||
                               value_obj.kind === 'UInt8Type' || value_obj.kind === 'Int8Type') {
                        // Keep as number for 32-bit and smaller numeric types
